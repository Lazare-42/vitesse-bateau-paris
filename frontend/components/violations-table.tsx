...
function mapsDirectionsUrl(
  ...
  return `https://www.google.com/maps/dir/${sLat},${sLon}/${eLat},${eLon}`;
}

function mapsPointUrl(lat: number, lon: number): string {
  return `https://www.google.com/maps?q=${lat},${lon}`;
}

function mapUrl(inf: Infraction): string {
  if (inf.start_lat === inf.end_lat && inf.start_lon === inf.end_lon) {
    return mapsPointUrl(inf.start_lat, inf.start_lon);
  }
...
export function ViolationsTable({ data }: { data: Infraction[] }) {
  if (data.length === 0) {
    return (
